...
/*
TODO:
- Find way to fill one extra buffer (make sure to apply fft again)
- Possibly change name of ft_create (seems vague)
*/

typedef struct {
    float* amplitude_size;
    float* amplitude;
    float* frequency;
} FreqDomain;
...
void fd_create(FreqDomain* fd, Wave* wave) {
    fd->amplitude = malloc(wave->frameCount * sizeof(fd->amplitude));
...
void ft_create(Wave* wave, FreqDomain* fd) {
    ...
    size_t amplitude_iterator = 0;
    short* wave_data_ptr = wave->data;
    
    for(wave_iterator = 0; wave_iterator < wave->frameCount - BUFFER_SIZE; ++wave_iterator) {
        for(size_t j = 0; j < BUFFER_SIZE; ++j) {
            PCM_buffer[j] = wave_data_ptr[wave_iterator++];
        }
        fft(PCM_buffer, fft_output, BUFFER_SIZE);
        for(size_t j = 0; j < BUFFER_SIZE; ++j) {
            fd->amplitude[amplitude_iterator++] = fft_output[j];
        }
    }
 
    for(size_t i = 0; i < wave->frameCount % BUFFER_SIZE; ++i) {
